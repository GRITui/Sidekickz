 ...
 * per-attempt `nonce` can live between the start and callback requests: it's
 * a short-lived, HMAC-signed token (payload + '.' + signature) verified with
 * LINE_LOGIN_STATE_SECRET, not a value looked up from server storage.
 */
...
// Signs {nonce, ts} into a compact, tamper-evident token carried through the
// `state` query param round-trip (browser -> LINE -> our callback).
export async function signState(nonce, secret) {
  const payload = bytesToBase64Url(new TextEncoder().encode(JSON.stringify({ nonce, ts: Date.now() })));
  const sig = bytesToBase64Url(new Uint8Array(await hmacSha256(secret, payload)));
  return `${payload}.${sig}`;
}

// Returns the embedded nonce if the signature is valid and the token isn't
// stale, else null (also null on any malformed input — never throws).
export async function verifyState(state, secret) {
  ...
  try {
    ...
    const { nonce, ts } = JSON.parse(new TextDecoder().decode(base64UrlToBytes(payload)));
