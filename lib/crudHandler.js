...
// bare JS ARRAY parameter as a Postgres ARRAY LITERAL ('{elem,elem,...}',
// each element independently stringified) — NOT as a JSON value. Every
// embedded-array field this API layer stores in a `jsonb` column (jobs'
// sub_tasks/milestones/options/items, invoices' line_items/
// payment_channels/slips, and more as they're added) hits this: the
// literal Postgres sends is not valid JSON syntax, so the insert/update
// throws and the caller sees a bare 502 with no indication why. Plain
// objects don't have this problem (the driver already JSON.stringifies a
// bare object), but stringifying both here is one simple, defensively
// correct rule instead of maintaining two. This was never caught by the
// test suite because every test fakes `sql` — no test here has ever
// round-tripped a real array value through a live Postgres connection.
// Exported (not just used internally) — api/migrate-upload.js, api/settings.js,
// and api/shop-public.js each build their own raw insert/update statement
// outside this shared factory and need the exact same fix, not a drifting
// copy of the same three-line function.
...
function buildSetClause(fields, body, startIndex) {
  ...
  for (const f of fields) {
    if (Object.prototype.hasOwnProperty.call(body, f)) {
      cols.push(`${f} = $${i}`);
      params.push(toParam(body[f] ?? null));
      i += 1;
    }
  }
  ...
  return async function handler(request) {
    ...
    try {
      ...
      if (request.method === 'POST') {
        ...
        const values = [body.cuid, userCuid, ...fields.map(f => toParam(body[f] ?? null)), new Date().toISOString()];
        const rows = await sql(
