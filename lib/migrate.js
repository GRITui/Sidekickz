 ...
 */
// P5 Phase 1 (auto-migrate on cold start, lib/autoMigrate.js): bump this
// EVERY time sql/schema-core.sql changes, in the SAME commit as the
// regenerate-the-embed step (tests/test-schema-sync.mjs's header has the
// exact `node -e ...` command that regenerates lib/schemaSql.js from the
// .sql file). lib/autoMigrate.js compares this string to the
// schema_meta.value it finds stamped on the database and re-applies the
// whole schema whenever they differ — every statement in the file is
// idempotent (create/alter ... if not exists), so re-applying an
// already-current schema is a harmless no-op, and the only failure mode
// this convention has is forgetting to bump it, which just means a real
// change silently doesn't get re-applied (under-applying), not that a
// no-op change gets mistakenly re-applied (over-applying — always safe).
//
// (lib/schemaSql.js — the verbatim embedded copy this bump must stay in
// sync with — exists only because every api/ handler runs on the Vercel
// Edge runtime, which has no `fs`; a vercel.json includeFiles-based read of
// the raw .sql file at request time was considered for this Phase 1 work
// and rejected as out of scope for that reason. tests/test-schema-sync.mjs
// is the drift guard between the two copies — keep it and the embed both.)
export const SCHEMA_VERSION = '2026-07-22.1';
...
export const REQUIRED_TABLES = [
  ...
  'bookings', 'team_members', 'order_requests', 'schema_meta',
];
